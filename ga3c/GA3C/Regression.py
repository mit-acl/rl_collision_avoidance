...
class Regression():
    def __init__(self, model, num_agents, actions):
        self.model = model
        self.file_dir = '/home/mfe/ford_ws/src/2017-avrl/src'
    ...
    def load_ped_data(self):
        try:
            if self.num_agents > 2: num_agents = 4
            elif self.num_agents == 2: num_agents = 2
            else: assert(0)
            ...
            dataset_ped_train = pickle.load(open(self.file_dir+\
                "/environment/Collision-Avoidance/CADRL/pickle_files/multi/%d_agents/2_3_4_agents_%scadrl_dataset_action_value_train.p"%(num_agents,prepend), "rb"))
            dataset_ped_test = pickle.load(open(self.file_dir+\
    ...
    def train(self, training_data, test_data):
        # training param
        batch_size = 2000; num_training_steps = 3000; PLOT_STEP = 100; 
        ...
        input_x = np.zeros((nb_training_examples,Config.FULL_STATE_LENGTH))
        input_x_test = np.zeros((nb_testing_examples,Config.FULL_STATE_LENGTH))
        ...
        for kk in xrange(initial_step, total_steps):
            ...
            trainer_id = 0; audio = None
            self.model.train(x, audio, y, a, trainer_id, learning_method='regression')
